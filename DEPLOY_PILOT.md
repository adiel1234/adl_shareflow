...
> **לפני גרסה חדשה:** קרא את [`PENDING_RELEASE.md`](PENDING_RELEASE.md) — רשימת תיקונים שנבדקו מקומית על הטלפון ועדיין לא ב־commit / לא ב־build מופץ.
...
git status
git add -A
git commit -m "פיילוט 1.0.9: תיקוני ליבה, משתתפים, סשן, אורח, wizard סגירה"
...
**גרסה:** וודא `1.0.9` build `63` ב-Xcode תואם ל-`pubspec.yaml`.
