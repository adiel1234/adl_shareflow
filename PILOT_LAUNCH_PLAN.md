...
עודכן: 11 אוגוסט 2026 · גרסת פיילוט: **1.0.9** · **`pubspec`:** `1.0.9+78` · **build 78 נבנה** · הודעת הרשאות אורח (מנהל בלבד) + תיקון `my_role` · IPA + APK מוכנים להפצה
...
> **מדיניות:** build **43** נבנה (17 יוני 2026). **build 44 בתור** — תיקוני join 404 + UX דיאלוג split_mode.
...
| **44** | ⏳ **בתור** | 1. **באג join 404** (backend תוקן; mobile: הסרת דיאלוג מג'וינר) 2. **UX split_mode**: דיאלוג למזמין בלבד 3. **UX הוצאה חלקית**: תווית «לא כולם שותפים» 4. **UX «שילמתי»**: דיאלוג אישור אצל החייב — צריך לעבור לנושה 5. **באג כפתור אישור חסר**: נושה רואה «ממתין לאישורך» אבל אין כפתור אשר/דחה — `canCreditorApprove` מחזיר false (`is_creditor_confirm` לא מגיע נכון בחבר↔חבר) | iOS + Android | `1.0.5+44` | הצטרפות; אישור תשלום — כפתור מוצג |
...
| [ ] | הוספת הוצאה + משתתפים | **3.1–3.3** | ✅ עבר | ✅ עבר |
...
| [ ] | יציאה מקבוצה | — | **דחוי** — לא מיושם; מנהל לא יכול לעזוב; פיצ'ר עתידי לחברים/אורחים | **דחוי** |
